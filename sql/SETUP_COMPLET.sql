...
-- ────────────────────────────────────────────────────────────
-- ÉTAPE 9 : MENU JOKER (lounge-bar-restaurant, Ngaliema)
INSERT INTO public.categories (nom, description, emoji, ordre, actif) VALUES
('Entrees et Snacks','Entrees et snacks a partager','🥗',1,true),
('Plats Principaux','Cuisine locale et internationale','🍽️',2,true),
('Grillades','Viandes et poissons grilles','🔥',3,true),
('Bar et Cocktails','Cocktails, vins, bieres','🍸',4,true),
('Desserts','Douceurs maison','🍰',5,true)
ON CONFLICT DO NOTHING;

INSERT INTO public.produits (nom, description, prix, categorie_id, disponible, ordre) VALUES
('Ailes de Poulet x6','Ailes epicees, sauce dip',8.00,(SELECT id FROM categories WHERE nom='Entrees et Snacks'),true,1),
('Salade Mixte','Tomates, concombres, oignons, vinaigrette',6.00,(SELECT id FROM categories WHERE nom='Entrees et Snacks'),true,2),
('Samoussa x4','Farce viande ou legumes',5.00,(SELECT id FROM categories WHERE nom='Entrees et Snacks'),true,3),
('Frittes Maison',NULL,3.00,(SELECT id FROM categories WHERE nom='Entrees et Snacks'),true,4),
('Poulet Braise + Riz','Demi-poulet braise, riz jaune, salade',14.00,(SELECT id FROM categories WHERE nom='Plats Principaux'),true,1),
('Riz au Gras Viande','Riz cuisines avec viande de boeuf, epices',10.00,(SELECT id FROM categories WHERE nom='Plats Principaux'),true,2),
('Spaghetti Bolognaise','Pates, viande hachee, sauce tomate',10.00,(SELECT id FROM categories WHERE nom='Plats Principaux'),true,3),
('Poulet Roti au Four','Poulet entier roti, herbes, pommes de terre',16.00,(SELECT id FROM categories WHERE nom='Plats Principaux'),true,4),
('Steak Boeuf sauce champignons','Entrecote 250g, sauce champignons, frites',18.00,(SELECT id FROM categories WHERE nom='Grillades'),true,1),
('Poisson Braise','Tilapia ou capitaine braise, citron-piment',12.00,(SELECT id FROM categories WHERE nom='Grillades'),true,2),
('Brochettes Mixtes','Boeuf, poulet, legumes grilles',14.00,(SELECT id FROM categories WHERE nom='Grillades'),true,3),
('Cocktail Maison Joker','Recette secrete du bar Joker',7.00,(SELECT id FROM categories WHERE nom='Bar et Cocktails'),true,1),
('Mojito','Rhum, menthe, citron vert, soda',8.00,(SELECT id FROM categories WHERE nom='Bar et Cocktails'),true,2),
('Whisky Joker Special','Blend selectionne, glacons',10.00,(SELECT id FROM categories WHERE nom='Bar et Cocktails'),true,3),
('Biere Primus 65cl',NULL,4.00,(SELECT id FROM categories WHERE nom='Bar et Cocktails'),true,4),
('Vin Rouge (verre)',NULL,7.00,(SELECT id FROM categories WHERE nom='Bar et Cocktails'),true,5),
('Jus Frais','Mangue, passion, ananas',4.00,(SELECT id FROM categories WHERE nom='Bar et Cocktails'),true,6),
('Tiramisu Maison',NULL,8.00,(SELECT id FROM categories WHERE nom='Desserts'),true,1),
